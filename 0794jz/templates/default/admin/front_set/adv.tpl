...
<table width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="cdcdcd">
  ...
  <tr>
    ...
    <td bgcolor="f5f5f5"><div align="center"><{$var.location}></div></td>
    <td bgcolor="f5f5f5"><div align="center"><{if $var.begin_time}><{$var.begin_time|date_format:"%Y-%m-%d %H:%M:%S"}><{else}>无限制<{/if}></div></td>
    <td bgcolor="f5f5f5"><div align="center"><{if $var.end_time}><{$var.end_time|date_format:"%Y-%m-%d %H:%M:%S"}><{else}>无限制<{/if}></div></td>
    <td bgcolor="f5f5f5"><div align="center"><a href="?act=add_adv&edit=mod&id=<{$var.id}>">编辑</a> | <a href="?act=adv&page=<{$pageinfo.current_page}>&id=<{$var.id}>&edit=del" onClick="return confirm('确定要删除<{$var.name}>吗？')">彻底删除</a></div></td>
  </tr>
  <{/foreach}>
  </table>
<{/if}>
...
&nbsp;&nbsp;｜&nbsp;&nbsp;
		  ...
		  <a href="?act=advs&page=<{$pageinfo.prev_page}>">上一页</a>
          <{else}>上一页<{/if}>
        &nbsp;&nbsp;|&nbsp;&nbsp;
            <{if $pageinfo.next_page}>
            <a href="?act=advs&page=<{$pageinfo.next_page}>">下一页</a>
            <{else}>下一页<{/if}>
            &nbsp;&nbsp;|&nbsp;&nbsp;
            <{if $pageinfo.current_page==$pageinfo.page_num}>
            最后一页
            <{else}>
            <a href="?act=advs&page=<{$pageinfo.page_num}>">最后一页</a>
            <{/if}>
