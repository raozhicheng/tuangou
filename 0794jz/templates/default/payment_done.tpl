...
<div class="body_area">
    <div id="content_box">
    	<div class="c_left">
        	<{nocache}>
        	<div class="payment">
                <div class="mess"><b>√&nbsp;<{$title}></b></div>
          	</div>
            <div class="deal_info">
            	<{foreach $order_deals as $var}>
                    <div class="deal_box red_l">
                    	<a href="<{$var.link}>"><{$var.name}></a>&nbsp;&nbsp;
                        <{if $var.buy_status==0}>
                        	已有<{$var.buy_count}>人参加
                        <{/if}>
                         <{if $var.buy_status==1}>
                        	已有<{$var.buy_count}>人参加，团购已成功
                        <{/if}>
                    </div>
                <{/foreach}>
            </div>
            <{if is_coupon==1}>
            <div class="coupon">
            	<div class="tit">你已获得团购券</div>
               	<div class="content">您可以在有效期内去商家开心享受超值服务！</div>
            </div>
            <{/if}>
            <{/nocache}>
