...
if(isset($_POST["picSubmit"])){
	...
	$fileUpload=new FileUpload(array("filePath"=>UPLOADPIC_PATH));
	if($fileUpload->uploadFile($_FILES["imgfile"]) < 0){
		echo $fileUpload->getErrorMsg();
	}else{
		$picName=$fileUpload->getNewFileName();
		
		if(Common::get_config("IS_MARK_OPEN")){	
			$gdImage=new GDImage(UPLOADPIC_PATH,$picName);
			$gdImage->makeThumb($_POST["w"], $_POST["h"],false); 
			global $waterText;
			$gdImage->waterMark($waterText);
			unlink(UPLOADPIC_PATH.$picName);
			$img_path=GALLERY_PATH.str_replace(".", "_new.", $picName);
		} else {
			$img_path=GALLERY_PATH.$picName;
		}
		
		$imgHtml='<img border=0; alt="" width="'.$_POST["w"].'" height="'.$_POST["h"].'" src="'.$img_path.'" />';	
